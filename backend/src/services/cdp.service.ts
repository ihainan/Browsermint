...
function broadcastFrame(
  ...
  if (opts.key) {
    // 高清静帧走整帧，并且**必须重置差分器**。
    //
    // 观看端收到它之后，手里的画面就是这张静帧了；而差分器的基准还停在之前那张流画面上。
    // 不重置的话，下一帧算出来的增量（含滚动位移）会被贴到静帧上——用户看到的是画面整体
    // 位移、顶部空出一条、底部被切掉，下一张整帧到来又恢复，来回跳（2026-08-04 用户实测）。
    //
    // 曾经按「分辨率一样就不用重置」来省一帧，那是看错了关键：要紧的不是尺寸对不对得上，
    // 而是**基准是不是同一张图**。静帧是重新截的（更清晰、质量也不同），本来就不是同一张。
    // 代价只有一张整帧，而且它发生在页面从静止重新动起来的那一刻——那时候本来就该重画。
    p.differ.reset();
