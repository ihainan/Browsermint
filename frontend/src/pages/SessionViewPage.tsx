...
import { sessionsApi, Session, SteelSessionDetails, BrowserTab } from "../api/client.ts";
import { Loader2, AlertCircle, Monitor, RefreshCw, Maximize2, X, Copy, Check, Plug, ExternalLink, ChevronLeft, ChevronRight, Globe, Plus } from "lucide-react";
...
function getFavicon(url: string): string | null {
  try {
    const u = new URL(url);
    if (u.protocol === "chrome:" || u.protocol === "about:") return null;
...
function resolveUrl(input: string): string {
  ...
  if (/^(https?|chrome|about|file):\/\//i.test(trimmed)) return trimmed;
...
function BrowserToolbar({
  ...
  useEffect(() => {
    let cancelled = false;
    const refresh = () => {
      sessionsApi.getTargets(sessionId, sessionToken)
        .then((r) => { if (!cancelled) setTabs(r.data.targets); })
        .catch(() => {});
    };
    refresh();
  ...
  useEffect(() => {
    ...
    const active = tabs.find((t) => t.targetId === pageId) ?? tabs[0];
    if (active) setUrlInput(active.url);
  }, [pageId, tabs, isEditingUrl]);

  const activeTab = tabs.find((t) => t.targetId === pageId) ?? tabs[0];

  const handleNavigate = (url: string) => {
    const target = activeTab;
    if (!target) return;
    sessionsApi.navigate(sessionId, sessionToken, { url: resolveUrl(url), targetId: target.targetId })
      .catch(() => {});
  };

  const handleNewTab = () => {
    sessionsApi.createTarget(sessionId, sessionToken).catch(() => {});
  };

  const handleCloseTab = (targetId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    // Don't close last tab
    if (tabs.length <= 1) return;
    sessionsApi.closeTarget(sessionId, sessionToken, targetId).catch(() => {});
  };

  const handleActivateTab = (targetId: string) => {
    if (targetId === activeTab?.targetId) return;
    sessionsApi.activateTarget(sessionId, sessionToken, targetId).catch(() => {});
  };

  const handleBack = () => {
    if (!activeTab) return;
    sessionsApi.goBack(sessionId, sessionToken, activeTab.targetId).catch(() => {});
  };

  const handleForward = () => {
    if (!activeTab) return;
    sessionsApi.goForward(sessionId, sessionToken, activeTab.targetId).catch(() => {});
  };

  const handleReload = () => {
    if (!activeTab) return;
    sessionsApi.browserReload(sessionId, sessionToken, activeTab.targetId).catch(() => {});
  };

  return (
    <div className="shrink-0 bg-[#e0e2e6] select-none" style={{ fontFamily: "system-ui, sans-serif" }}>
      {/* Tab bar */}
      <div className="flex items-end pl-1 pt-1.5 overflow-x-auto gap-px" style={{ height: 34 }}>
        {tabs.map((tab) => {
          const isActive = tab.targetId === activeTab?.targetId;
          const favicon = getFavicon(tab.url);
          const title = tab.title || tab.url || "New Tab";
          return (
            <button
              key={tab.targetId}
              onClick={() => handleActivateTab(tab.targetId)}
              className={clsx(
                "group relative flex items-center gap-1.5 px-3 h-full min-w-0 max-w-[200px] shrink-0 text-left transition-colors",
                "rounded-t-lg text-xs",
                isActive
                  ? "bg-white text-gray-900 shadow-sm z-10"
                  : "bg-[#d0d3d9] hover:bg-[#d8dadc] text-gray-600"
              )}
              style={{ minWidth: 80 }}
            >
              {favicon ? (
                <img src={favicon} alt="" className="w-3.5 h-3.5 shrink-0" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
              ) : (
                <Globe size={12} className="shrink-0 text-gray-400" />
              )}
              <span className="flex-1 truncate leading-none">{title}</span>
              {tabs.length > 1 && (
                <span
                  onClick={(e) => handleCloseTab(tab.targetId, e)}
                  className={clsx(
                    "shrink-0 w-4 h-4 flex items-center justify-center rounded-full transition-colors",
                    isActive
                      ? "text-gray-500 hover:bg-gray-200 hover:text-gray-900"
                      : "text-gray-500 opacity-0 group-hover:opacity-100 hover:bg-gray-300"
                  )}
                >
                  <X size={10} />
                </span>
        ...
        <button
          ...
          className="flex items-center justify-center w-8 h-7 mb-0.5 rounded-full text-gray-500 hover:bg-[#c8cacd] transition-colors shrink-0"
          title="New tab"
        >
          <Plus size={14} />
        </button>
      ...
      <div className="flex items-center gap-1 px-2 pb-1.5 pt-1">
        <button
          onClick={handleBack}
          className="w-7 h-7 flex items-center justify-center rounded-full text-gray-600 hover:bg-[#c8cacd] transition-colors shrink-0"
          title="Back"
        >
          <ChevronLeft size={16} />
        </button>
        ...
        <div className="flex-1 flex items-center bg-white rounded-full px-3 h-7 gap-1.5 border border-white focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
          <Globe size={12} className="text-gray-400 shrink-0" />
          <input
            ref={urlRef}
            type="text"
            value={urlInput}
            className="flex-1 text-xs text-gray-800 outline-none bg-transparent min-w-0"
            onChange={(e) => setUrlInput(e.target.value)}
            onFocus={() => { setIsEditingUrl(true); urlRef.current?.select(); }}
